...
Commontator.configure do |config|
  ...
  config.user_name_proc = lambda { |user| user.nickname }
  ...
  config.user_link_proc = lambda { |user, app_routes| "/users/"+user.id.to_s }
  ...
  #              user, 1, :s => 60, :d => 'mm') }
  config.user_avatar_proc = lambda { |user, view| user.avatar.url(:menu) }
